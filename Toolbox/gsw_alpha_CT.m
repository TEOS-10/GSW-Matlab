function alpha_CT = gsw_alpha_CT(SA,CT,p)

% gsw_alpha_CT                thermal expansion coefficient with respect to 
%                               Conservative Temperature (48-term equation)
%==========================================================================
%
% USAGE:  
%  alpha_CT = gsw_alpha_CT(SA,CT,p), or equivalently
%     alpha = gsw_alpha(SA,CT,p)
% 
%  Note that gsw_alpha(SA,CT,p) is identical to gsw_alpha_CT(SA,CT,p).  
...
%  expression for density in terms of SA, CT and p (McDougall et al., 2013)
%   
...
if ~(nargin == 3)
    error('gsw_alpha_CT:  Requires three inputs')
end %if
...
if (mt ~= ms | nt ~= ns)
    error('gsw_alpha_CT: SA and CT must have same dimensions')
end
...
elseif (ms == mp) & (ns == np)
    ...
    error('gsw_alpha_CT: Inputs array dimensions arguments do not agree')
end %if
