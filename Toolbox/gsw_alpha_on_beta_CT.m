function alpha_on_beta_CT = gsw_alpha_on_beta_CT(SA,CT,p)

% gsw_alpha_on_beta_CT                        alpha/beta (48-term equation)
%==========================================================================
% 
% USAGE:  
% alpha_on_beta_CT = gsw_alpha_on_beta_CT(SA,CT,p), or equivalently
%    alpha_on_beta = gsw_alpha_on_beta(SA,CT,p)
%
%  Note that gsw_alpha_on_beta_CT(SA,CT,p) is identical to 
%  gsw_alpha_on_beta(SA,CT,p).  The extra "_CT" emphasises that the 
...
%
% OUTPUT:
%  alpha_on_beta_CT  =  thermal expansion coefficient with respect to                          
...
if ~(nargin == 3)
   error('gsw_alpha_on_beta_CT:  Requires three inputs')
end %if
if ~(nargout == 1)
   error('gsw_alpha_on_beta_CT:  Requires one outputs')
end %if
...
if (mt ~= ms | nt ~= ns)
    error('gsw_alpha_on_beta_CT: SA and CT must have same dimensions')
end
...
elseif (ms == mp) & (ns == np)
    ...
    error('gsw_alpha_on_beta_CT: Inputs array dimensions arguments do not agree')
end %if
...
alpha_on_beta_CT = gsw_alpha_on_beta(SA,CT,p);

if transposed
   alpha_on_beta_CT = alpha_on_beta_CT.';
end
