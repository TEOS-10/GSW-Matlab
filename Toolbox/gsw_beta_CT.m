...
%
% USAGE:  
%  beta_CT = gsw_beta_CT(SA,CT,p), or equivalently
%     beta = gsw_beta(SA,CT,p)
% 
%  Note that gsw_beta(SA,CT,p) is identical to gsw_beta_CT(SA,CT,p).  
...
%
% VERSION NUMBER: 3.0 (23rd March, 2010)
%
% REFERENCES:
...
if ~(nargin == 3)
   error('gsw_beta_CT:  Requires three inputs')
end %if
...
if (mt ~= ms | nt ~= ns)
    error('gsw_beta_CT: SA and CT must have same dimensions')
end
...
elseif (ms == mp) & (ns == np)
    ...
    error('gsw_beta_CT: Inputs array dimensions arguments do not agree')
end %if
