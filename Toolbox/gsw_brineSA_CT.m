function brineSA_CT = gsw_brineSA_CT(CT,p,saturation_fraction)

% gsw_brineSA_CT        Absolute Salinity of seawater at the freezing point
%==========================================================================
%
% USAGE:
%  brineSA_CT = gsw_brineSA_CT(CT,p,saturation_fraction)
%
% DESCRIPTION:
...
if ~(nargin == 2 | nargin == 3) 
   error('gsw_brineSA_CT:  Requires either two or three inputs')
end %if
...
if (saturation_fraction < 0 | saturation_fraction > 1)
   error('gsw_brineSA_CT: saturation_fraction MUST be between zero and one.')
end
...
elseif (mt == mp) & (nt == np)
    ...
    error('gsw_brineSA_CT: Inputs array dimensions arguments do not agree')
end %if
