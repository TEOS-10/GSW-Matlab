function brineSA_t = gsw_brineSA_t_poly(t,p,saturation_fraction)

% gsw_brineSA_t_poly                   Absolute Salinity of seawater at the
%                                                     freezing point (poly)
%==========================================================================
%
% USAGE:
%  brineSA_t = gsw_brineSA_t_poly(t,p,saturation_fraction)
%
% DESCRIPTION:
...
if ~(nargin == 2 | nargin == 3) 
   error('gsw_brineSA_t_poly:  Requires either two or three inputs')
end %if
...
if (saturation_fraction < 0 | saturation_fraction > 1)
   error('gsw_brineSA_t_poly: saturation_fraction MUST be between zero and one.')
end
...
elseif (mt == mp) & (nt == np)
    ...
    error('gsw_brineSA_t_poly: Inputs array dimensions arguments do not agree')
end %if
...
if ~isempty(Itw)
    brineSA_t(Itw) = Nan;       % If the t input is too warm, then there is 
