...
dvhatden_dCTdCT = a02 + 2*a03*CT ...
           + SA.*(a05 + sqrtSA.*(a07 + 2.*a08*CT)) ...
            + p.*(a10 + a13*p);
     
dvhatden_dCTdSA = a04 + a05*CT ...
 + (3/2)*sqrtSA.*(a06 + CT.*(a07 + a08*CT))...
                + a11*p;
...
dvhatnum_dCTdSA = a18 + CT.*(a19 + CT.*(a20 + a21*CT)) ...
 + (3/2)*sqrtSA.*(a22 + CT.*(a23 + CT.*(a24 + a25*CT))) ...
            + p.*(a29 + p*a32);
...
dvhatnum_dSAdSA = (1./(2*sqrtSA)).*(b16 + CT.*(b17 + CT.*(b18 + CT.*(b19 + b20*CT)))) + b21;

dvhatden_dSAdSA = (1./(2*sqrtSA)).*(b04 + CT.*(b05 + CT.*(b06 + b07*CT)));
...
p2e = (dvhatden_dCT.*dvhatden_dSA)./(v_hat_denominator.^2);

Part2 = p2a - p2b - p2c - p2d + 2*p2e;
...
Part3 = p3a - 2*p3b - p3c + 2.*p3d.^2;

cabbeling = Part1 - 2*(factor2a./factor2b).*Part2 + (factor3a./factor3b).^2.*Part3;
