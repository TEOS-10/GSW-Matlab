function [dSA_dCT_frazil, dSA_dP_frazil, dCT_dP_frazil] = gsw_frazil_ratios(SA,p,w_Ih)

% gsw_frazil_ratios              ratios of SA, CT and P changes when frazil
%                                  ice forms due to the adiabatic change in 
%                                 pressure of a mixture of seawater and ice
%==========================================================================
%
% USAGE:
%  [dSA_dCT_frazil, dSA_dP_frazil, dCT_dP_frazil] = gsw_frazil_ratios(SA,p,w_Ih)
%
% DESCRIPTION:
...
if ~(nargin == 3 )
    error('gsw_frazil_ratios: Requires three inputs')
end
...
if (mw_Ih ~= ms | nw_Ih ~= ns)
    error('gsw_frazil_ratios: SA and w_Ih must have same dimensions')
end
...
elseif (ms == mp) & (ns == np)
    ...
    error('gsw_frazil_ratios: Inputs array dimensions arguments do not agree; check p')
end 
...
[CTf_SA, CTf_P] = gsw_CT_freezing_first_derivatives(SA,p);
