...
% gsw_steric_height                                   steric height anomaly
%                                                        (48-term equation)
%==========================================================================
%
% USAGE:  
%  steric_height = gsw_steric_height(SA,CT,p,p_ref)
%
% DESCRIPTION:
...
%  expression for specific volume of McDougall et al. (2011). Note that the 
...
%  in McDougall et al. (2011) and IOC et al. (2010).  For dynamical 
...
if ~(nargin == 4)
   error('gsw_steric_height:  Requires four inputs')
end %if

unique_p_ref = unique(p_ref);
if ~isscalar(unique_p_ref)
    error('gsw_steric_height: The reference pressure p_ref must be unique')
end
clear p_ref
p_ref = unique_p_ref;

if p_ref < 0
    error('gsw_steric_height: The reference pressure p_ref must be positive')
end

[InegSA] = find(SA < 0);
if ~isempty(InegSA)
    error('gsw_steric_height: The Absolute Salinity must be positive!')
end
...
if (ms~=mt) | (ns~=nt)
    error('gsw_steric_height: SA & CT need to have the same dimensions')
elseif (ms*ns == 1)
    error('gsw_steric_height: There must be at least 2 values')
end

if (mp == 1) & (np == 1)              % p scalar - fill to size of SA
    error('gsw_steric_height: need more than one pressure')
...
elseif (ms == mp) & (ns == np)
    ...
    error('gsw_steric_height: Inputs array dimensions arguments do not agree')
end %if
...
if max(p(:)) < p_ref
    error('gsw_steric_height: The reference pressure p_ref is deeper than all bottles')
end
