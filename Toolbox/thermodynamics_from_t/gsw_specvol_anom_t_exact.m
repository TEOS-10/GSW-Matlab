...
%
% USAGE:  
%  specvol_anom_t_exact = gsw_specvol_anom_t_exact(SA,t,p)
%
% DESCRIPTION:
...
if ~(nargin == 3)
   error('gsw_specvol_anom_t_exact:  Requires three inputs')
end %if
...
if (mt ~= ms | nt ~= ns )
    error('gsw_specvol_anom_t_exact: SA and t must have same dimensions')
end
...
elseif (ms == mp) & (ns == np)
    ...
    error('gsw_specvol_anom_t_exact: Inputs array dimensions arguments do not agree')
end %if
...
specvol_anom_t_exact = gsw_gibbs(n0,n0,n1,SA,t,p) - gsw_gibbs(n0,n0,n1,SSO,t_zero,p);
